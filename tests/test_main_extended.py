...
class TestInteractivePrompt:
    ...
    def test_single_mode_with_files(self, tmp_path):
        ...
        with (
            patch("builtins.input", side_effect=["3", "n", "1"]),
            patch("sys.stdin") as mock_stdin,
            patch("modules.cli.src.surface_cli_interactive_controller.DEFAULT_TODO", tmp_path / "todo"),
        ):
            mock_stdin.isatty.return_value = True
            result = _interactive_prompt()
            assert result is not None
            assert result.mode == "single"
            assert result.input_path == (todo / "task.md").resolve()
    ...
    def test_single_mode_no_files(self, tmp_path):
        ...
        with (
            ...
            patch("sys.stdin") as mock_stdin,
            patch("modules.cli.src.surface_cli_interactive_controller.DEFAULT_TODO", empty),
        ):
            mock_stdin.isatty.return_value = True
            result = _interactive_prompt()
            assert result is not None
    ...
    def test_single_mode_invalid_choice(self, tmp_path):
        ...
        (todo / "task.md").write_text("task")
        with (
            patch("builtins.input", side_effect=["3", "n", "abc"]),
            patch("sys.stdin") as mock_stdin,
            patch("modules.cli.src.surface_cli_interactive_controller.DEFAULT_TODO", tmp_path / "todo"),
        ):
            mock_stdin.isatty.return_value = True
    ...
    def test_single_mode_out_of_range(self, tmp_path):
        ...
        (todo / "task.md").write_text("task")
        with (
            patch("builtins.input", side_effect=["3", "n", "99"]),
            patch("sys.stdin") as mock_stdin,
            patch("modules.cli.src.surface_cli_interactive_controller.DEFAULT_TODO", tmp_path / "todo"),
        ):
            mock_stdin.isatty.return_value = True
