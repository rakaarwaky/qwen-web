...
(tests/fixtures/qwen_fixture.html). Locks selectors, MutationObserver injection,
...
class TestDetectResponseMutation:
    def test_detects_mutation_when_node_added(self, client, page):
        page.evaluate("""() => {
            setTimeout(() => {
                const log = document.getElementById('chatLog');
                const reply = document.createElement('div');
                reply.className = 'message-text';
                reply.textContent = 'This is a long mutation response generated by Qwen AI model.';
                log.appendChild(reply);
            }, 100);
        }""")
        res = client._detect_response_mutation(timeout_sec=5)
        assert res is not None
        assert "mutation response" in res

    def test_returns_none_on_timeout_or_error(self, client, page, monkeypatch):
        def _boom(*a, **k):
            raise Exception("Playwright evaluate error")

        monkeypatch.setattr(page, "evaluate", _boom)
        res = client._detect_response_mutation(timeout_sec=1)
        assert res is None


class TestAdaptivePoll:
    def test_adaptive_poll_finds_response_text(self, client, page):
        page.evaluate("""() => {
            const log = document.getElementById('chatLog');
            const reply = document.createElement('div');
            reply.className = 'message-text';
            reply.textContent = 'Adaptive poll response text from chat model.';
            log.appendChild(reply);
        }""")
        res = client._adaptive_poll(timeout_sec=3)
        assert res == "Adaptive poll response text from chat model."

    def test_adaptive_poll_returns_none_on_timeout(self, client, page):
        res = client._adaptive_poll(timeout_sec=1)
        assert res is None
...
class TestSendFilePipeline:
    ...
    def test_send_file_raises_timeout_when_no_response(self, client, page, tmp_path, monkeypatch):
        monkeypatch.setattr(client, "reset_page", lambda: None)
        monkeypatch.setattr(page, "goto", lambda *a, **k: None)
        monkeypatch.setattr(client, "_detect_response_mutation", lambda t: None)
        monkeypatch.setattr(client, "_adaptive_poll", lambda t: None)
