...
def _validate_min(name: str, value: float | int, minimum: float | int) -> None:
    """Raise ValueError if a configuration value is below its minimum."""
    if value < minimum:
        raise ValueError(f"{name} must be >= {minimum}, got {value}")


def _default_log_path() -> Path:
    """Resolve the default log directory without importing constants."""
    state_home = (
        Path(os.environ["XDG_STATE_HOME"]) / "qwen-web"
        if os.environ.get("XDG_STATE_HOME")
        else Path.home() / ".local/state/qwen-web"
    )
    return state_home / "log"
...
class AppConfig:
    ...
    def status_path(self) -> Path:
        ...
        return self.log_path / _STATUS_FILENAME
