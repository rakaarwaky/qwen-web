"""Capabilities: QwenClient legacy-compat facade (AES403).

Maintains the historical QwenClient API surface used by the behavior-lock
test suite: QwenClient(ctx, cfg), .send_file(), .reset_page(), .start(),
.stop(), plus the private delegate helpers. Internally it composes the
capability modules directly.
"""
...
from modules.shared.src.taxonomy_domain_error import QwenCliError
